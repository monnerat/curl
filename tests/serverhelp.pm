...
sub serverfactors {
    ...
    if($server =~
        ...
        /^(dns|tftp|sftp|socks|ssh|sieve|rtsp|gopher)(\d*)(-ipv6|)$/) {
        $proto  = $1;
        $idnum  = ($2 && ($2 > 1)) ? $2 : 1;
        $ipvnum = ($3 && ($3 =~ /6$/)) ? 6 : 4;
    }
...
sub servername_str {
    ...
    die "unsupported protocol: '$proto'" unless($proto &&
        ($proto =~ /^(((DNS|FTP|HTTP|HTTP\/2|HTTP\/3|IMAP|POP3|GOPHER|SMTP|HTTPS-MTLS)S?)|(TFTP|SFTP|SIEVE|SOCKS|SSH|RTSP|DICT|SMB|SMBS|TELNET|MQTT|MQTTS))$/));
