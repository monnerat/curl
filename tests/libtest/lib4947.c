...
static CURLcode test_lib4947(const char *URL)
{
  CURL *curl = NULL;
  ...
  /* Shorten buffers. */
  easy_setopt(curl, CURLOPT_BUFFERSIZE, 8192L);
  easy_setopt(curl, CURLOPT_UPLOAD_BUFFERSIZE, 8192L);

  /* Set the URL that targets the remote script. */
  easy_setopt(curl, CURLOPT_URL, URL);

  /* Prepare to read data (for upload). */
  easy_setopt(curl, CURLOPT_INFILESIZE, (long) FILESIZE);
  easy_setopt(curl, CURLOPT_READFUNCTION, read_cb);
  easy_setopt(curl, CURLOPT_READDATA, &count);
