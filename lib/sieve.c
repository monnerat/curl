...
static void sieve_easy_reset(struct SIEVE *sieve)
{
  ...
  curlx_safefree(sieve->owner);
  curlx_safefree(sieve->scriptname);
  curlx_safefree(sieve->custom);
  curlx_safefree(sieve->custom_params);

  /* Clear the transfer mode for the next request. */
  sieve->transfer = PPTRANSFER_BODY;
}

static void sieve_easy_dtor(const void *key, size_t klen, void *entry)
{
...
static void sieve_conn_dtor(const void *key, size_t klen, void *entry)
{
                                      ...
                                      struct connectdata *conn)
{
  ...
  if(owner) {
    if(strcmp(owner, Curl_creds_sasl_authzid(conn->creds))) {
      struct Curl_creds *creds = conn->creds;
      result = Curl_creds_create(Curl_creds_user(creds),
                                 Curl_creds_passwd(creds),
                                 Curl_creds_oauth_bearer(creds),
                                 owner,
                                 Curl_creds_sasl_service(creds),
                                 creds ? creds->source : CREDS_NONE,
                                 &conn->creds);
    }
    curlx_free(owner);
    if(result)
                                 ...
                                 const struct bufref *initresp)
{
  ...
  if(!ir) {
    ...
    result = Curl_pp_sendf(data, &sievec->pp, "AUTHENTICATE \"%s\" \"=\"",
                           mech);
                                           ...
                                           struct sieve_conn *sievec)
{
  ...
  CURLcode result = CURLE_OK;
  struct Curl_creds *creds = data->conn->creds;
                           ...
                           bool premature)
{
  ...
  if(!sieve)
    ...
    if(status) {
      /* Mark for closure. */
      CURL_TRC_M(data, "sieve done with bad status");
      connclose(conn);
